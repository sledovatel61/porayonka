...
def create_controls_tab(page: ft.Page) -> ft.Column:
    ...
    # Раунд 39 (задача 1): «Очистить» ВНУТРИ ЕДИНОГО глобального календаря —
    # существующий контракт setter(Optional[str]), второй календарь НЕ
    # создаётся. Кнопка видна только там, где очистка разрешена (can_clear —
    # конечная дата); у «Дата получения»/«Срок»/дат пунктов её нет, чтобы не
    # менять их семантику.
    ...
    def _open_global_cal(setter, current_iso=None, can_clear=False):
        ...
        # Раунд 39 (задача 1): «Очистить» — только у поля с разрешённой
        # очисткой (сейчас это конечная дата периодического контроля).
    ...
    def _open_detail(ctl: Optional[Control]):
        ...
        # Раунд 39 (задача 1): ЯВНАЯ команда очистки. Корень дефекта: у поля
        # конечной даты не было ни крестика, ни «Очистить» — дату нельзя было
        # снять, только перезаписать. Паттерн — как в create_russian_date_field
        # (внутренний IconButton перехватывает клик раньше on_click поля).
        ...
        def _clear_end(e=None):
            # Раунд 39 (задача 1): обнуляется ТОЛЬКО end_date — due_date,
            # period_days, tasks и milestones не трогаем.
            _set_end(None)
        end_clear_btn.on_click = _clear_end
        # Раунд 39 (задача 1): can_clear=True — «Очистить» в едином календаре.
        ...
        def _refresh_cycle_hint():
            try:
                ...
                traceback.print_exc()
        # Раунд 39 (задача 1): предыдущий тип — обнулять end_date только на
        # реальном ПЕРЕХОДЕ periodic -> one-time, а не при любом on_change.
    ...
    # ════════════════════════════════════════════════════════════════
    # Корень: браузер НЕ отдаёт серверу локальный путь, у
    # FilePickerResultEvent.files[*].path = None — код брал e.path и молча
    # выходил. Файл нужно ЗАГРУЗИТЬ: picker.upload([FilePickerUploadFile(name,
    # page.get_upload_url(name, expires))]) и дождаться on_upload с
    # progress == 1.0 и error is None; файл ляжет в FLET_UPLOAD_DIR/<имя>
    # (flet/fastapi/flet_upload.py). Каталог и ключ подписи задаёт main.py
    #::_entry() — без них PUT /upload не работает вовсе.
    # Импортируем ТОЛЬКО существующий физический .xlsx; фиктивные пути
    # запрещены. Ожидание НЕ блокирующее: тело обработчика Flet 0.23.2
    # исполняется под общим UI-lock (ui/update_lock.py), on_upload приходит
    # тем же путём -> wait внутри on_result был бы ДЕДЛОКОМ. Цепочка:
    # on_result -> upload -> on_upload -> preview.
    _import_pending = {"name": None, "active": False}

    def _import_log(msg: str):
        """ASCII-safe запись в error.log (frozen console=False: print уходит
        в devnull, исключение теряется бесследно)."""
        try:
