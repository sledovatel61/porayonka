    ...
    return ft.Container(
        ...
        content=ft.Column(
            controls=[
                ft.Row(
                    controls=[
                        ft.Icon(icon, size=14, color=COLORS.get("stat_blue_text", "#60a5fa")),
                        ft.Text(
                            item.name,
                            size=10,
                            weight=ft.FontWeight.W_600,
...
def create_criminalists_summary_panel(
    ...
    header = ft.Container(
        content=ft.Row(
            controls=[
                ...
                ft.Text("Сводка по криминалистам", size=14,
    ...
    items = sorted(collection.template.items, key=lambda item: item.order)
    aggregate_controls = [
        ft.Text("Итоги по пунктам", size=12, weight=ft.FontWeight.BOLD,
                color=COLORS.get("stat_blue_text", "#60a5fa"))
    ]
    if items:
    ...
    legend = ft.Row(
        controls=[
            ft.Row(
                controls=[
                    ft.Icon(ft.icons.TASK_ALT, size=13,
                            color=COLORS.get("received_text", "#4ade80")),
                    ft.Text("сначала полностью сдавшие", size=10,
                            color=COLORS.get("text_secondary", "#94a3b8")),
                ],
                spacing=4,
                tight=True,
