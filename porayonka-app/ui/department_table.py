...
def _make_number_badge(dept: Department) -> ft.Container:
    """Кружок с номером."""
    text  = "—" if dept.is_ovd else str(dept.id)
...
def _make_dept_row(
    ...
    row = ft.Container(
        content=ft.Row(
            controls=[
                ft.Container(
                    content=_make_number_badge(dept),
                    width=56,
                    alignment=ft.alignment.center,
                ),
                ft.Container(
...
def create_department_table(
    ...
    def _handle_status_click(dept: Department) -> None:
        print(f"[HANDLE] {dept.name}: {dept.status}", end=" -> ")
        dept.toggle_status()
        print(f"{dept.status}")
