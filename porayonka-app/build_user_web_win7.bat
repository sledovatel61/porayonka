...
echo.
echo [Шаг 5/7] PyInstaller Porayonka_User_Web.spec (3-7 минут)...
"%W7PY%" -m PyInstaller Porayonka_User_Web.spec --noconfirm --clean --log-level WARN
if errorlevel 1 (
    echo.
...
python tools\make_web_manifest.py --venv %W7VENV% --exe "dist\Порайонка_Пользователь_Web.exe" --out "dist\manifest_win7_web.txt"
if errorlevel 1 (
    echo.
