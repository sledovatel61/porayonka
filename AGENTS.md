...
**Остающаяся проблема:**
- Плашки в `ft.GridView` отображаются как пустые серые прямоугольники правильного размера, но внутри них не видно текста, иконок, прогресса и кнопок.
- Логика поиска/фильтрации работает (количество плашек меняется), то есть объекты создаются и добавляются в сетку.
- Необходимо либо починить рендеринг content внутри `ft.GridView`, либо заменить `GridView` на другой layout (`Column` + `Row(wrap=True)` или ручная сборка рядов).
...
ВАЖНО:
- Начни с минимального воспроизводимого примера: одна плашка с одним ft.Text
  внутри GridView. Если и он пустой — проблема в GridView/Container/Stack.
- Если одна плашка с ft.Text работает — добавляй по одному элементу, пока не
  найдёшь ломающий контрол.
- Если GridView не работает в принципе — замени его на Row(wrap=True) или на
  ручную сборку рядов (4 плашки в Row, несколько Row в Column).
