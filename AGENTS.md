...
└── porayonka-app/          # Основное Python + Flet приложение
    ...
    └── ui/                 # UI-компоненты Flet
        ...
        └── zonal/            # Вкладка «Зональные криминалисты»
            ...
            ├── criminalist_card.py    # Карточка криминалиста (раскрывающаяся)
            ├── zone_manager.py        # Редактор зон обслуживания
            ├── add_criminalist_modal.py  # Добавление/редактирование криминалиста
            └── summary_panel.py       # Общая сводка по сбору данных
```
...
- **Карточки криминалистов**:
  - Раскрывающиеся карточки.
  - Редактирование ФИО, примечания, закреплённых отделов.
  - Удаление и добавление новых криминалистов.
- **Общая сводка** по всем пунктам и криминалистам.
...
- **Поддержка Flet 0.23.2**: используется `hint_style=ft.TextStyle(...)` вместо устаревшего `hint_text_color`. См. `fix_flet.py`.
...
|------|------------|-------|
| `fix_flet.py` | Заменяет `hint_text_color="..."` на `hint_style=ft.TextStyle(color="...")` в 5 файлах | Совместимость с Flet 0.23.2 |
...
5. **Дублирование импорта `get_initial_criminalists`**: в `main.py` импортирован из `core.zonal_constants` и сразу используется. В `zonal_tab.py` импортирован дважды (из `core.zonal_data` и `core.zonal_constants`) — не критично, но стоит знать.
