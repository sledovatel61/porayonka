...
- В коммите `2ad4ee9` `GridView` заменён на `Row(wrap=True)`, а внутри плашки всем контролам заданы конечные размеры. Патч применён, требуется проверка в реальном GUI Windows.
...
ВАЖНО:
- Патч от предыдущего агента уже применён в коммите 2ad4ee9: GridView заменён
  на Row(wrap=True), плашкам заданы фиксированные размеры. Начни с проверки
  текущего состояния, а не с нуля.
- Если плашки всё ещё пустые/серые — временно упрости content плашки до одного
  ft.Text, чтобы понять, рендерится ли хоть что-то внутри Row(wrap=True).
- Если текст появляется — наращивай layout постепенно и найди, какой контрол
  ломает рендер (ProgressBar, IconButton, Container с ink/shadow, и т.д.).
- Если и один ft.Text не появляется — проблема глубже (Container/Stack/ink/shadow
  или Flet 0.23.2 на Windows). Попробуй ft.Card, ft.ElevatedButton или другой
  корневой контейнер.
- Если layout работает, но плашки всё ещё неудобны (слишком много/мало в ряд,
  обрезается текст) — адаптируй константы ширины и высоты.
