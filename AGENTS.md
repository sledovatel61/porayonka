...
## 18. Выполнено: редизайн вкладки «Зональные» (фаза 3)

Задача из раздела 17 реализована. Ниже — что было не так, что изменено и какие
инварианты нельзя ломать.

### 18.1 Почему плашки были гигантскими и пустыми

Обе проблемы имели одну причину — **неограниченные (unbounded) constraints по высоте**.

1. **Гигантская высота.** Плашки лежали в `ft.ResponsiveRow` внутри корневой
   `ft.Column(scroll=ft.ScrollMode.AUTO)`. `ResponsiveRow` задаёт ребёнку только
   *ширину* (через `col`), а высоту передаёт от родителя. Родитель — ScrollView,
   то есть высота там бесконечная. Внутри плашки стоял
   `ft.Column(expand=True)` и распорка `ft.Container(expand=True)`
   («чтобы кнопки были внизу»). Вертикальный `expand` в бесконечной высоте
   растягивается до максимума, поэтому `height=224` у контейнера-плашки
   переставал соблюдаться, и каждая плашка занимала целый экран.
   16 таких плашек = многометровый скролл.

2. **Пустой серый вид.** Это следствие того же самого. Flutter, получая
   `Expanded` внутри unbounded-высоты, схлопывает дочерний layout:
   рисуется только фон и рамка контейнера (`bgcolor=COLORS["card"]` — серый),
   а текст ФИО/процентов/отделов получает нулевые/некорректные constraints
   и не отрисовывается. Дополнительно `ft.Text(expand=True)` внутри `Row`
   и `max_lines` работали непредсказуемо, т.к. родительская ширина тоже
   вычислялась некорректно.

**Как исправлено:** внутри плашки не осталось ни одного `expand`.
Все размеры заданы явно: контейнер `360x88`, три колонки фиксированной
ширины (`28 / 234 / 58`), тексты с явным `width` и `overflow=ELLIPSIS`,
`ProgressBar` с явными `width`/`height`. Сетка переведена с `ResponsiveRow`
на `ft.Row(wrap=True)`: у детей фиксированный размер, поэтому Wrap корректно
переносит строки, а высота сетки считается по содержимому.

### 18.2 Что изменено

| Файл | Изменение |
|---|---|
| `ui/zonal/criminalist_tile.py` | Полностью переписан. Компактная плашка `360x88` без единого `expand`. Содержимое: иконка-переключатель активности, ФИО, статус, закреплённые отделы (кратко, полный список в tooltip), прогресс-бар, процент, меню `PopupMenuButton`. Клик по плашке -> `create_form_input_modal`. |
| `ui/zonal/collapsible.py` | **Новый файл.** `create_collapsible_section(title, icon, body, expanded=False, subtitle=None)` — сворачиваемая секция. Свёрнутое состояние = `visible=False` у тела, поэтому секция занимает только высоту заголовка. |
| `ui/zonal/summary_panel.py` | Переписан. Добавлена `create_summary_line(collection)` — однострочная сводка (активных / сдано / общий прогресс) для основного экрана. `create_summary_panel(collection)` теперь подробная сводка для свёрнутой секции; `ResponsiveRow` убран. |
| `ui/zonal/template_builder.py` | Убран собственный градиентный заголовок (его рисует сворачиваемая секция), emoji в кнопках заменены на `ft.icons.*`, уменьшены отступы и высоты полей. |
| `ui/zonal/zonal_tab.py` | Пересобран корневой layout, весь UI-текст переведён на русский, все `print()` переведены в ASCII. Сетка `ResponsiveRow` -> `Row(wrap=True)`. Экспорт в Excel перенесён в тулбар. Добавлено сообщение «Ничего не найдено» для пустого фильтра. |
| `core/zonal_data.py` | Только тексты, попадающие в буфер обмена (`build_non_submitters_text`) — с транслита на русский. Логика не тронута. |
| `core/zonal_constants.py`, `core/zonal_exporter.py`, `main.py` | Только ASCII-логи (были русские `print()`, потенциальный `UnicodeEncodeError` на cp1251). |

### 18.3 Итоговая структура вкладки

```
Column(scroll=AUTO)                      <- единственный ScrollView вкладки
├── Row(wrap)      однострочная сводка: активных / сдано / общий прогресс
├── Column         секция «Настройка сбора данных»  (свёрнута по умолчанию)
├── Column         секция «Подробная сводка»        (свёрнута по умолчанию)
├── Row(wrap)      тулбар: поиск, фильтры, добавить, копировать, экспорт
├── Row(wrap)      счётчик «Показано: N из M» + подсказка
└── Row(wrap)      сетка плашек 360x88 (дети фиксированного размера)
```

### 18.4 Инварианты (не ломать)

- Внутри `criminalist_tile.py` **никогда** не добавлять `expand=True` —
  плашки живут внутри Wrap, который не умеет раздавать Expanded-детям
  конечные constraints. Только явные `width`/`height`.
- Сетка плашек — `Row(wrap=True)`, а не `ResponsiveRow`. Смена обратно на
  `ResponsiveRow` вернёт «гигантские пустые плашки».
- Единственный ScrollView вкладки — её корневая `Column`. Не добавлять
  вложенный `scroll` в секции.
- Ширина плашки `TILE_WIDTH` и ширины колонок в `criminalist_tile.py`
  связаны формулой `_MAIN_COL_W`. Меняя `TILE_WIDTH`, не трогать формулу.
- `print()` — только ASCII. UI-текст — только русский.
...
Функционал, проверенный после редизайна: добавление / редактирование / удаление
криминалиста, переключение активности, поиск, все три фильтра, сохранение /
загрузка / очистка формы, сброс данных, экспорт в Excel, копирование списка
не сдавших, открытие модального окна заполнения по клику на плашку.
